...
describe('lib', () => describe('movie', () => {
  ...
  describe('update', () => {
    ...
    it('updates one row from table `movie`, by `id`', async ({context}: Flags) => {
      const anyId = 123
      if(!isContext(context)) throw TypeError()
      const anyName = 'Mitchel'
      const anySynopsis = 'some synopsis from movie'
      const anyReleasedAt = new Date('2015-03-04T00:00:00.000Z')
      const anyRuntime = 123456

      context.stub.knex_update.resolves()

      await update(anyId, anyName,anySynopsis,anyReleasedAt,anyRuntime)
      sinon.assert.calledOnceWithExactly(context.stub.knex_from, 'movie')
      sinon.assert.calledOnceWithExactly(context.stub.knex_where, { id: anyId })
      sinon.assert.calledOnceWithExactly(context.stub.knex_update, { name: anyName })
      sinon.assert.calledOnceWithExactly(context.stub.knex_update, { synopsis: anySynopsis })
      sinon.assert.calledOnceWithExactly(context.stub.knex_update, { releasedAt: anyReleasedAt })
      sinon.assert.calledOnceWithExactly(context.stub.knex_update, { runtime: anyRuntime })
    })
    ...
    ; [0, 1].forEach( rows =>
      it(`returns ${!!rows} when (${rows}) row is found and deleted`, async ({context}: Flags) => {
        ...
        const result = await update(anyId, anyName,anySynopsis,anyReleasedAt,anyRuntime)
        expect(result).to.be.boolean()
        expect(result).equals(!!rows)
      }))
  ...
  describe('create', () => {
    ...
    it('insert one row into table `movie`', async ({context}: Flags) => {
      ...
      await create(anyName,anySynopsis,anyReleasedAt,anyRuntime)
      sinon.assert.calledOnceWithExactly(context.stub.knex_into, 'movie')
      sinon.assert.calledOnceWithExactly(context.stub.knex_insert, { name: anyName, synopsis:anySynopsis, releasedAt:anyReleasedAt, runtime:anyRuntime})
    })
    ...
    it('returns the `id` created for the new row', async ({context}: Flags) => {
      ...
      const result = await create(anyName,anySynopsis,anyReleasedAt,anyRuntime)
