...
describe('lib', () => describe('actor', () => {
  ...
  describe('update', () => {
    ...
    it('updates one row from table `actor`, by `id`', async ({context}: Flags) => {
      ...
      const anyBio = 'some bio from actor'
      const anyBornAt = new Date('2015-03-04T00:00:00.000Z')
      ...
      sinon.assert.calledOnceWithExactly(context.stub.knex_update, { name: anyBio })
      sinon.assert.calledOnceWithExactly(context.stub.knex_update, { name: anyBornAt })
    })
